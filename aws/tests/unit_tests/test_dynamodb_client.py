...
# Add the babies lambda directory to the path (using babies as reference for shared utilities)
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'lambdas', 'babies'))

from lambdas.shared.dynamodb_client import DynamoDBClient
...
class TestDynamoDBClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock boto3 resource BEFORE creating client instance
        self.boto3_patcher = patch('lambdas.shared.dynamodb_client.boto3.resource')
        self.mock_dynamodb_resource = self.boto3_patcher.start()
        
        # Create client instance AFTER mocking boto3
        self.client = DynamoDBClient()
        
        # The client.dynamodb should now be the mock
        self.mock_dynamodb = self.client.dynamodb
...
class TestDynamoDBClientIntegration(unittest.TestCase):
    ...
    @patch('lambdas.shared.dynamodb_client.boto3.resource')
