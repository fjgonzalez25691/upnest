...
def lambda_handler(event, context):
    ...
    logger.info(f"Growth Data Service - Event: {json.dumps(event, default=str)}")
...
def get_baby_growth_data(event):
    ...
    try:
        ...
        logger.info(f"Retrieved {len(records)} growth data records for baby {baby_id}")
...
def list_growth_data(event):
    ...
    try:
        ...
        logger.info(f"Retrieved {len(records)} growth data records for user {user_id}")
        return success_response({
            'records': records,
            'count': len(records)
        })
...
def update_growth_data(event):
    ...
    try:
        ...
        if 'measurementDate' in body:
            ...
            expression_values[':measurementDate'] = {'S': body['measurementDate']}
        ...
        if 'measurementSource' in body:
            if body['measurementSource']:
                ...
                expression_values[':measurementSource'] = {'S': body['measurementSource']}
            else:
