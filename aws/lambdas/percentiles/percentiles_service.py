...
# Add shared utilities to path - handle both local and Lambda environments
try:
    ...
    try:
        ...
        def success_response(data=None, message="Success"):
            return {'statusCode': 200, 'body': json.dumps({'success': True, 'data': data, 'message': message})}
        def bad_request_response(message="Bad request"):
            return {'statusCode': 400, 'body': json.dumps({'success': False, 'error': message})}
        def unauthorized_response(message="Unauthorized"):
            return {'statusCode': 401, 'body': json.dumps({'success': False, 'error': message})}
...
def calculate_zscore(value, L, M, S):
    ...
    try:
        ...
        raise ValueError(f"Invalid LMS values or measurement: L={L}, M={M}, S={S}, value={value}")
...
def find_lms_values(df, age_days):
    ...
    if row.empty:
        ...
        if age_days < df['Day'].min():
            ...
            logger.warning(f"Age {age_days} days is younger than available data. Using minimum age.")
        elif age_days > df['Day'].max():
            # Use the oldest age available
            row = df.loc[df['Day'] == df['Day'].max()]
            logger.warning(f"Age {age_days} days is older than available data. Using maximum age.")
        else:
...
def calculate_percentile(event):
    ...
    try:
        ...
        logger.info(f"Calculated {measurement_type} percentile for user {user_id}: {percentile:.2f}%")
        
        return success_response({
            'measurementType': measurement_type,
            'value': float(value),  # Return original value (e.g., grams for weight)
...
def lambda_handler(event, context):
    ...
    logger.info(f"Percentiles Service - Event: {json.dumps(event, default=str)}")
