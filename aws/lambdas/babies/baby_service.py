...
try:
    ...
    def created_response(data=None, message="Created"):
        return {'statusCode': 201, 'body': json.dumps({'success': True, 'data': data, 'message': message})}
    ...
    def not_found_response(message="Not found"):
        return {'statusCode': 404, 'body': json.dumps({'success': False, 'error': message})}
    def forbidden_response(message="Forbidden"):
        return {'statusCode': 403, 'body': json.dumps({'success': False, 'error': message})}
    ...
    def method_not_allowed_response(message="Method not allowed"):
        return {'statusCode': 405, 'body': json.dumps({'success': False, 'error': message})}
...
class BabyService:
    ...
    def __init__(self, event, context):
        ...
        logger.info(f"BabyService initialized for user: {self.user_id}, request: {self.request_id}")
    ...
    def _get_baby_by_id(self, baby_id):
        ...
        try:
            ...
            if baby.get('userId') != self.user_id:
                raise PermissionError("Access denied: baby belongs to another user")
